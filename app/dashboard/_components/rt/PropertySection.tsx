            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  style={{ outline: "none" }}
                >
                  {dwellingDistribution.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[(index + 2) % COLORS.length]} style={{ outline: "none" }} />
                  ))}
                </Pie>
                <Tooltip formatter={(value) => [`${value || 0} Hunian`, "Jumlah"]} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-gray-border bg-gray-page-bg p-4">
              <div className="flex items-center justify-between text-xs text-gray-secondary-text mb-1">
                <span className="font-semibold uppercase tracking-wider">Okupansi Kamar Sewa</span>
              ...
              {dwellingDistribution.map((item, idx) => (
                <div key={item.type} className="flex justify-between items-center">
                  <div className="flex items-center gap-1.5">
                    <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: COLORS[(idx + 2) % COLORS.length] }} />
      ...
      <div className="border-t border-gray-divider pt-4 mt-4 grid grid-cols-2 gap-4 text-center">
        <div>
          ...
          <h4 className="text-lg font-extrabold text-gray-heading-main mt-1">
            {dwellingDistribution.reduce((sum, item) => sum + item.count, 0)} Unit
          </h4>
        ...
        <div>
          ...
          <h4 className="text-lg font-extrabold text-gray-heading-main mt-1">
            {dwellingDistribution.find((d) => d.type === "Rumah Kosong")?.count || 0} Unit
          </h4>
