...
interface DeleteWargaMemberModalProps {
  ...
  onCustomDelete?: (member: WargaFamilyMember, note: string) => Promise<void>;
}
...
export const DeleteWargaMemberModal: React.FC<DeleteWargaMemberModalProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      if (onCustomDelete) {
        await onCustomDelete(member, inactiveNote);
      ...
      <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl border border-gray-border bg-gray-card shadow-2xl p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-secondary-text mt-1 leading-relaxed">
            Apakah Anda yakin ingin menonaktifkan <span className="font-bold text-gray-heading-main">{member.name}</span> (NIK: {member.nik}) dari daftar Kartu Keluarga?
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="Contoh: Pindah domisili, menikah dan membuat KK baru, dll."
