...
import { Edit2, Trash2, FileText, ExternalLink, ArrowDownRight } from "lucide-react";
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-gray-border bg-gray-sidebar-hover text-xs font-bold text-gray-secondary-text">
              ...
              <th className="py-3.5 px-4 text-center">Nota / Bukti</th>
            ...
            {items.length > 0 ? (
              items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-sidebar-hover/20 transition-colors">
                  ...
                  <td className="py-3.5 px-4 font-semibold">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold bg-rose-50 text-rose-800 border border-rose-200">
                      {item.category}
                    </span>
                  </td>

                  <td className="py-3.5 px-4 max-w-xs">
                    <span className="text-gray-heading-main font-bold block truncate">
                      {item.description || "-"}
                    </span>
                  </td>
              ...
              <tr>
                <td colSpan={6} className="py-12 text-center text-gray-placeholder font-medium">
                  Belum ada data pengeluaran kas yang ditemukan.
                </td>
              </tr>
            )}
          </tbody>
